...
- As of this 2026-07-12 checkpoint, the release-centric album/track candidate is complete on branch `improve/album-track-publication`, based on the current `main`, but has not been deployed to staging or production.
- The candidate makes album publication grant inherited playability to current and future member tracks without listing them as standalone releases. Album pages own their ordered track listings; direct track/audio access, public playlists, recommendations, and smart-playlist eligibility use effective album-or-track availability. The persistent player exposes separately grouped album, playlist, and standalone-track choices.
- The candidate's administrator track library defaults to collapsible album groups. Grouped mode deliberately loads every matching track so an album cannot be hidden by a saved per-page limit; ordinary pagination returns when grouping is disabled. Audio-health results poll every five seconds, retries clear stale state, and a draft album's configured embedded/gallery cover is evaluated independently from its intentionally private public URL.
...
- Local validation passed: 118 application tests with 727 assertions; Pint across 144 PHP files; both tracked Unraid Compose examples; an isolated PostgreSQL legacy-data upgrade rehearsal; and a rebuilt development stack with migration exit `0`, healthy web/worker services, and public album, track, audio, search, player, cache-policy, and readiness smoke checks.
...
- The remaining release gate is to submit this branch through pull request and CI, let the merge publish an immutable digest, deploy that digest to staging, and complete the focused acceptance checks in [UNRAID.md](UNRAID.md). Do not promote to production until that replacement digest is accepted on staging.

## Known follow-up work

1. **Release and staging acceptance:** required TOTP MFA, private publication-aware media delivery, scheduled-publication enforcement, security headers, accessible navigation/lightbox/player behavior, bundled admin visuals, private-media migration, and artwork variants passed the earlier development and staging/test acceptance. The replacement album-first candidate is locally complete but still needs PR/CI, a new immutable digest, and focused staging acceptance before approval.
