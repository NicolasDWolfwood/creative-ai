...
If testing fails, do not promote the digest. Put the previously known good digest back into staging and repeat Down and Up. Code rollback is safe only while database migrations remain backward-compatible. The inherited-album and Journal lifecycle releases keep legacy publication fields as compatibility mirrors for this read-only rollback path; the connected-stories tables are additive and ignored by an older image. Avoid music publication, Journal publication, or Journal connection edits while an older image is temporarily restored.
