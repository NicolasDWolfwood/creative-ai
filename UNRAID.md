...
9. Create an incomplete Journal draft with a private brief and notes. Confirm it can be saved and previewed by the administrator, remains `404` anonymously, and does not expose the private fields. Create an active Journal template, open **Story opportunities**, and create a draft from a public source with that template. Confirm the source is connected at position one but its fields, files, and publication state are unchanged; the new post is still Draft; and the source leaves the opportunity list. Complete the readiness blockers, schedule the post in UTC, verify it remains private before the due time and appears on the Journal page, RSS, and sitemap after that time without a scheduler job. Unpublish it and confirm the page and cover immediately return `404` anonymously.
...
11. Preview a Journal draft and confirm the response is private, no-store, noindex, and free of private brief/notes. In **Journal templates**, create an active outline using `{{ source_title }}` and `{{ source_type }}`. From **Story opportunities**, create a draft for one public source with optional shared-tag copying; confirm the generated title/body/brief, Draft state, position-one connection, removal from the opportunity queue, and that the source record and publication state did not change. Schedule a completed post in UTC, verify anonymous page/cover/feed/sitemap visibility changes only when due, then unpublish it and verify access is revoked everywhere. On a published test post, add a shared tag and ordered public plus draft media connections. Confirm the public post preserves the public-media order, omits the draft source, appears reciprocally on each public media page, and links to a public tag archive that contains no private or future records.
...
If testing fails, do not promote the digest. Put the previously known good digest back into staging and repeat Down and Up. Code rollback is safe only while database migrations remain backward-compatible. The inherited-album and Journal lifecycle releases keep legacy publication fields as compatibility mirrors for this read-only rollback path; the connected-stories and planning-tools tables plus opportunity indexes are additive and ignored by an older image. Avoid music publication, Journal publication, Journal connection, or planning-template edits while an older image is temporarily restored.
...
For the inherited-album and Journal lifecycle releases, retain the expand-compatible migrations and do not run `migrate:rollback`. The connected-stories and planning-tools additions can also remain applied because older images ignore their tables and indexes. Treat music and Journal publication, connection, and planning administration as read-only while the older image is running, then restore the current image before making publication, scheduling, connection, template, or album-membership changes.
