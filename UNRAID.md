...
10. Generate or refresh one published automatic collection with member publication enabled. Confirm reviewed draft members remain off **All artwork** but appear inside the collection, have canonical detail/image URLs, use collection-context navigation, and produce a non-placeholder cover without requiring Featured status. Apply another matching AI result and verify it stays outside the fixed public snapshot until an explicit confirmed refresh. Review the refresh impact counts, then unpublish or remove the final grant and verify anonymous detail/media return `404`. Select mixed artwork states and verify the four publication/Featured bulk actions report only changed rows, preserve publication dates on unpublish, synchronize the published-only smart set, retain legitimate collection-only access, and never publish through a Featured change.
