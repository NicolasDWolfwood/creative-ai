...
6. Open **Showcase → Artwork signatures**, upload the reviewed transparent monochrome PNG, and save the defaults. Confirm the transparency, black, and white previews are correct and no visible signature pixel touches the asset edge. Upload a clean artwork master with **Automatic** treatment and publish it only after the worker reports a complete large, display, and thumbnail set. Verify all three public-safe renditions return `200`, the clean master remains administrator-only, and the anonymous master request returns `404`. Exercise light and dark target corners and confirm Automatic selects a readable contrasting tone; exercise one manual tone or corner override. Change one global placement value and confirm saving alone leaves existing bytes active, then use **Queue stale renditions** and confirm the prior complete set remains available until the immutable replacement succeeds. Verify a migrated **Already embedded** artwork retains its existing paths and is not signed a second time. On its public detail page, confirm **View larger image** opens the signed large rendition in a closable modal without leaving the site; test its close button, backdrop, Escape key, focus return, mobile layout, and the scoped right-click, drag, and long-press deterrents. Confirm separately that direct HTTP access to public renditions remains possible by design and never reaches the clean master.
7. Open **AI & Automation → AI providers** with Ollama selected. Confirm the panel offers the server and separate image/Journal model choices without request-timeout, context-length, or keep-alive controls, then inspect the provider. Run one queued action and confirm the worker consumes it without sending Ollama `keep_alive` or `num_ctx` overrides.
8. Apply reviewed AI metadata to at least two draft artworks with usable variants. Confirm each suggestion has no more than three broad reusable tags for prominent visible subjects, objects, or settings; omits color, style, mood, and medium labels; and reuses an accurate recurring subject label when one fits. In **Applied tags**, manually add one reusable tag and remove another, save, and confirm the automatic/live-smart matches update without deleting either global tag or silently changing a fixed public snapshot. Run **Generate automatic** with **Publish generated collections** and **Make matched artwork public inside these collections** enabled. Confirm the resulting collection has a real artwork cover even when no member is Featured, its selected artwork appears inside the collection with a **Via collection** administrator status, and it remains absent from **All artwork** while its canonical detail and protected image URLs return `200`. Apply a newly matching AI tag to another draft and confirm background processing does not add it to the public collection. Run the explicit **Refresh automatic** action, review the added/removed/visible counts, and confirm the new match appears only after that approval. Unpublish the collection and verify collection-only detail and media return `404`; another live granting collection or standalone publication must keep them available. Filter the artwork table to **Not in any collection**, select at least two results, and run **Create draft collection from selected**. Before confirming, add one selected artwork to another collection in a second tab; verify the action skips that stale selection, creates a private manual collection from the remainder, and changes no publication or Featured state. In **Artwork**, select a mixed draft, future-scheduled, and current group; verify **Publish selected now** makes the first two current without rewriting the existing past date, then verify **Remove standalone publication** preserves dates and reports any member that remains available through a live collection. Confirm the two Featured bulk actions change only Featured status, not standalone publication or collection membership. In a controlled state with one due Featured-only artwork as the sole homepage candidate, load the unfiltered homepage twice and confirm the same image is used for the UTC day. Confirm its homepage display succeeds while generic original/display/thumbnail media, canonical detail, **All artwork**, collection pages, and sitemap remain closed; then remove Featured and confirm the cached homepage choice is withdrawn. Repeat with a due standalone-published candidate and confirm it is eligible through ordinary publication.
...
10. Generate or refresh one published automatic collection with member publication enabled. Confirm reviewed draft members remain off **All artwork** but appear inside the collection, have canonical detail/image URLs, use collection-context navigation, and produce a non-placeholder cover without requiring Featured status. Confirm newly analyzed artwork receives no more than three broad reusable tags for prominent visible subjects, objects, or settings; omits color, style, mood, and medium labels; and prefers accurate recurring subject/general vocabulary. In **Applied tags**, add one shared tag and remove another; verify automatic/live-smart matching updates, global tags remain intact, and a fixed public snapshot still requires explicit refresh. Apply another matching AI result and verify it stays outside the fixed public snapshot until an explicit confirmed refresh. Review the refresh impact counts, then unpublish or remove the final grant and verify anonymous detail/media return `404`. Use the **Not in any collection** filter and create a draft collection from selected results; make one selection stale in a second tab and confirm it is skipped, the remaining membership is correct, and no publication or Featured flag changes. Select mixed artwork states and verify the four publication/Featured bulk actions report only changed rows, preserve publication dates on unpublish, synchronize the published-only smart set, and retain legitimate collection-only access. In a controlled one-candidate state, verify the unfiltered homepage keeps the same due Featured-only hero for the UTC day while its generic media, detail, archive, collection, recommendation, and sitemap surfaces remain closed; remove Featured and confirm the cached choice is withdrawn. Confirm a due standalone-published artwork is also eligible for the homepage and ordinary public routes.
...
For the inherited-album, artwork-signature, and Journal lifecycle releases, retain the expand-compatible migrations and do not run `migrate:rollback`. The connected-stories, planning-tools, revision, redirect, and soft-delete additions can also remain applied because older images ignore their columns, tables, and indexes. Treat artwork uploads, deletion, signature/rendition controls, music, and all Journal administration as read-only while the older image is running, then restore the current image before making publication, scheduling, slug, trash, history, connection, template, or album-membership changes.
...
Inspect the artwork's signature/rendition status and the web and worker logs. Confirm its private master exists and, for Automatic, Black, or White treatment, that **Showcase → Artwork signatures** has a validated private PNG mask. A first incomplete rendition set is intentionally unavailable to anonymous requests; a failed refresh keeps the prior complete set live. Retry the row or selected artwork after correcting the source or signature settings. Do not diagnose this by restoring the retired root static-site folders.

Before rolling back to an image that predates artwork signatures, pause artwork uploads, deletion, and signature administration, then let the current worker consume or cancel all queued rendition jobs. Leave the additive migration applied and do not run `migrate:rollback`: dropping `master_path` would discard the database references needed to resume clean-master rendering. Confirm every artwork has a non-null public-safe large rendition. The older image ignores the private-master and signature metadata and continues using that active public-safe image, but it cannot maintain the new rendering contract.
