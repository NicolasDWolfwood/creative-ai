...
If testing fails, do not promote the digest. Put the previously known good digest back into staging and repeat Down and Up. Code rollback is safe only while database migrations remain backward-compatible. The inherited-album and Journal lifecycle releases keep legacy publication fields as compatibility mirrors for this read-only rollback path; the collection-member flag, connected-stories, planning-tools, revision, redirect, and soft-delete additions are ignored by an older image. Collection-only members therefore disappear rather than becoming standalone-public. Trashed rows remain private because their legacy publication state is cleared. Avoid collection publication/membership edits, music publication, and every Journal edit—including slugs, trash, history, connections, or templates—while an older image is temporarily restored.
