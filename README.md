...
Public gallery tiles open their signed display rendition directly in the site's accessible lightbox; the former separate circular quick-view control is unnecessary. The popup includes **View artwork details**, which opens the existing artwork page and preserves collection context, while the tile's ordinary link remains a no-JavaScript fallback. On an artwork detail page, **View larger image** opens the signed large rendition in the same lightbox rather than replacing the page with a bare image. The modal closes through its visible close control, backdrop, or Escape and restores focus to the link. Public artwork previews also suppress the ordinary image context menu, dragging, and mobile touch callout as a casual copying deterrent. This is not a security boundary: any image a browser can display can still be recovered through browser tools or a screenshot. The meaningful safeguards are the baked signature, capped public renditions, and the clean master's administrator-only route.
...
An artwork's **Publish as standalone artwork** control governs ordinary standalone publication and the **All artwork** archive. A published collection can separately enable **Make member artwork public through this collection**. That grant gives its selected members publication-aware detail and image URLs while they remain absent from the standalone archive, mirroring album-only tracks. Removing the final current collection grant withdraws a collection-only artwork; another current grant or deliberate standalone publication keeps it available.

The unfiltered homepage hero uses a separate, deliberately narrow rule: it chooses one usable image from artwork that is either Featured or standalone-published and whose standalone publish date is absent or due. The choice is stable for the UTC day and rotates fairly across the eligible pool instead of changing on every request; a cached choice is revalidated so withdrawn or missing media is replaced. Featured-only artwork is delivered through the dedicated homepage-display route but remains closed on generic artwork media, detail, archive, collection, recommendation, and sitemap surfaces. Filtered gallery and collection views retain their contextual hero behavior.
...
Artwork AI metadata generates at most three broad, reusable tags for prominent visible subjects, objects, or settings. It prefers an existing recurring subject vocabulary when that vocabulary fits and omits separate color, style, mood, and medium tags. The artwork editor's **Applied tags** section remains directly editable: administrators can search, add, create, or remove shared tags without deleting the global tag records. These manual changes update automatic and live smart matching while preserving the explicit snapshot gate for draft-capable public collections. Applying a later AI suggestion replaces the curated applied-tag list, so approve AI suggestions before making deliberate manual refinements.

The artwork library can filter **Collection membership** to **In any collection** or **Not in any collection**. **Create draft collection from selected** transactionally rechecks the selection and attaches only artwork that is still in no collection when the action runs. It creates a private manual collection only when at least one eligible item remains, skips stale selections that gained membership elsewhere, and never changes collection publication, member publication, standalone publication, or Featured state.
...
The artwork library selection menu supports **Publish selected now**, **Remove standalone publication**, **Mark selected as Featured**, and **Remove Featured from selected**. Publication batches are transactional and refresh live smart membership once for the complete selection. Publishing now replaces future dates but preserves existing past dates; removing standalone publication preserves dates and collection memberships, so the confirmation and result explicitly report artwork that remains available through a published collection. Featured actions never change standalone publication or collection membership, but a due Featured artwork is eligible for the narrow homepage hero described above.
