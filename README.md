...
Artwork AI metadata asks for a small set of broad, reusable subject and theme tags, includes a broad parent alongside an optional accurate specific label, and prefers an existing recurring vocabulary when it fits. The artwork editor's **Applied tags** section is also directly editable: administrators can search, add, create, or remove shared tags without deleting the global tag records. These manual changes update automatic and live smart matching while preserving the explicit snapshot gate for draft-capable public collections. Applying a later AI suggestion replaces the curated applied-tag list, so approve AI suggestions before making deliberate manual refinements.
